...
test("final-only usage keeps live values estimated, then reconciles to exact usage", () => {
  ...
  const betweenChunks = state.snapshot(1_500);
  assert.equal(betweenChunks.output.value, live.output.value);
  assert.equal(betweenChunks.tps.value, live.tps.value);
...
test("prompt display stays cumulative across turns and uses wall-clock TPS", () => {
  const state = new CyberEditorState();
  state.onPromptStart(0);
  state.onAgentStart(0);
  ...
  const afterFirstTurn = state.snapshot(1_100);
  assert.equal(afterFirstTurn.inputValue, 10);
  assert.equal(afterFirstTurn.output.value, 20);
